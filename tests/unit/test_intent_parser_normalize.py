"""_normalize_cast_spell_target (graph/nodes/intent_parser.py) is a pure,
...
from src.graph.nodes.intent_parser import _normalize_cast_spell_target
...
def test_promotes_a_stray_params_target_to_the_top_level_field() -> None:
    ...
    normalized = _normalize_cast_spell_target(action)
...
def test_ignores_stray_params_target_for_a_different_verb() -> None:
    # Only cast_spell gets this treatment - some other verb's own params
    # entry that happens to be named "target" isn't this bug's shape.
    action = ParsedAction(
        actor="thorin",
        verb="attack",
        raw_text="I attack",
