...
- "cast_spell": the player casts a spell that isn't a direct attack (e.g. a buff, a utility spell). Set "target" to who it's aimed at, the same top-level field "attack" uses (most spells need one) - never nest it inside "params". Set "item_or_spell" to the spell name if given.
